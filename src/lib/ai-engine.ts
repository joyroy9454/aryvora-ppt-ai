...
export function buildImagePrompt(
  ...
  const styleModifiers: Record<string, string> = {
    corporate: "professional corporate style, clean vector illustration, blue tones",
    academic: "academic illustration, detailed diagram style, muted colors",
    startup: "modern tech illustration, vibrant gradient, bold dynamic",
    minimal: "minimalist flat icon style, simple geometric shapes, clean lines",
    dark: "dark theme illustration, neon glow accents, futuristic",
    seminar: "friendly educational illustration, soft colors, approachable",
    marketing: "vibrant marketing graphic, eye-catching, energetic colors",
    research: "scientific data visualization, precise, technical illustration",
    education: "colorful educational illustration, engaging, playful",
    portfolio: "creative artistic illustration, stylish, elegant",
  };

  const style = styleModifiers[templateId] || styleModifiers.corporate;
  const keywordStr = keywords.slice(0, 4).join(", ");

  return `Create a professional presentation slide image: "${slideHeading}".
Style: ${style}.
Elements to include: ${keywordStr || slideHeading}.
The image should be suitable for a ${slideType} slide, centered composition,
no text or letters in the image, high contrast, presentation-ready quality.`;
}

export function shouldGenerateImage(slideType: string): boolean {
  return (
    slideType === "title" ||
    slideType === "image-left" ||
    slideType === "image-right" ||
    slideType === "divider" ||
    slideType === "closing"
  );
}

...
Generate this ONE slide. Make it focused, impactful, and presentation-ready. Follow the ${analysis.presentationCategory} design profile — this should feel like a ${analysis.presentationCategory} presentation.`;
  ...
  try {
    ...
    return {
      ...
      bullets: (content.bullets && content.bullets.length > 0) ? content.bullets : plan.keyPoints,
      leftCol: content.leftCol && content.leftCol.length > 0 ? content.leftCol : undefined,
      rightCol: content.rightCol && content.rightCol.length > 0 ? content.rightCol : undefined,
      quote: content.quote,
      author: content.author,
      stats: content.stats && content.stats.length > 0 ? content.stats : undefined,
      timeline: content.timeline && content.timeline.length > 0 ? content.timeline : undefined,
      process: content.process && content.process.length > 0 ? content.process : undefined,
      chart: content.chart && content.chart.length > 0 ? content.chart : undefined,
